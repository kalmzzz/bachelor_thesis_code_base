...
def generate_single_image_pertubed_dataset_gpu(model_path, output_name, target_class, new_class, EPS, ITERS, pertube_count, take_optimal=True):
    print("[ Initialize.. ]")
    model, model_complete = get_model(model_path)
    data_path = "madry_data/release_datasets/normal_CIFAR/"
    train_data = ch.load(os.path.join(data_path, f"CIFAR_ims_normal")).to(device)
    train_labels = ch.load(os.path.join(data_path, f"CIFAR_lab_normal")).to(device)
    train_dataset = TensorDataset(train_data, train_labels, transform=transforms.ToTensor())
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=len(train_dataset), shuffle=False, num_workers=0)
    ...
    for idx, (input, target) in tqdm(enumerate(train_dataset)):
        ...
        if target == new_class:
            advs = adversary.perturb(torch.unsqueeze(input, 0), new_class_input.to(device))
            new_images[idx] = advs
            activation = model(advs)
            dataset_loss_dict[idx] = kl_div_loss(activation, new_class_input).to('cpu')
    ...
    torch.save(new_images_final, 'madry_data/release_datasets/pertubed_CIFAR/CIFAR_ims_'+str(output_name))
