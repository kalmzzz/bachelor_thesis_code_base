...
def generate_pertubed_dataset_main(target_class, new_class, eps, iter, dataset_name, inf=False, pertube_count = 1.0):
    ...
    basic_net = get_model()
    if inf:
        adversary = LinfPGDAttack(basic_net, loss_fn=nn.CrossEntropyLoss(), eps=eps, nb_iter=iter, eps_iter=0.00784, rand_init=True, clip_min=0.0, clip_max=1.0, targeted=True)
    else:
    ...
    for idx, (input, target) in enumerate(train_dataset):
        if target == target_class:
            ...
            if current_pertube_count <= np.floor((pertube_count * len(train_dataset))):
    ...
    print("\n[ Saving Dataset.. ]")
    if inf:
        torch.save(new_images, 'madry_data/release_datasets/perturbed_CIFAR/CIFAR_ims_Linf_'+str(dataset_name)+'_'+str(pertube_count)+'pert_'+str(iter)+'iters_'+str(eps)+'eps')
        torch.save(new_labels, 'madry_data/release_datasets/perturbed_CIFAR/CIFAR_lab_Linf_'+str(dataset_name)+'_'+str(pertube_count)+'pert_'+str(iter)+'iters_'+str(eps)+'eps')
    else:
        torch.save(new_images, 'madry_data/release_datasets/perturbed_CIFAR/CIFAR_ims_L2_'+str(dataset_name)+'_'+str(pertube_count)+'pert_'+str(iter)+'iters_'+str(eps)+'eps')
        torch.save(new_labels, 'madry_data/release_datasets/perturbed_CIFAR/CIFAR_lab_L2_'+str(dataset_name)+'_'+str(pertube_count)+'pert_'+str(iter)+'iters_'+str(eps)+'eps')
