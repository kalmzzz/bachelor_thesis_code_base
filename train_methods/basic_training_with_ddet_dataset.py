...
#file_name = 'basic_training_with_ddet_dataset'
...
def get_loaders():
    data_path = "madry_data/release_datasets/ddet_CIFAR/"
    train_data = ch.load(os.path.join(data_path, f"CIFAR_ims"))
    train_labels = ch.load(os.path.join(data_path, f"CIFAR_lab"))
    train_dataset = TensorDataset(train_data, train_labels, transform=transform_train) #transform muss wieder auf train gestellt werden

    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=128, shuffle=True, num_workers=4)
    return train_loader


def ddet_training_main(epochs, learning_rate):
