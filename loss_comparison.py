...
if __name__ == "__main__":
    ...
    axes[1][1].set_title(str(LOSS)+" with $\epsilon=2.0$")
    axes[1][1].axis('off')
    axes[1][2].imshow(softmaxed1, cmap="cool")
    axes[1][2].set_title(str(LOSS)+" with $\epsilon=2.0$")
    axes[1][2].axis('off')
    ...
    axes[2][1].set_title(str(LOSS)+" with $\epsilon=1.0$")
    axes[2][1].axis('off')
    axes[2][2].imshow(softmaxed2, cmap="cool")
    axes[2][2].set_title(str(LOSS)+" with $\epsilon=1.0$")
    axes[2][2].axis('off')
    ...
    axes[3][1].set_title(str(LOSS)+" with $\epsilon=0.75$")
    axes[3][1].axis('off')
    axes[3][2].imshow(softmaxed3, cmap="cool")
    axes[3][2].set_title(str(LOSS)+" with $\epsilon=0.75$")
    axes[3][2].axis('off')
    ...
    axes[4][1].set_title(str(LOSS)+" with $\epsilon=0.5$")
    axes[4][1].axis('off')
    axes[4][2].imshow(softmaxed4, cmap="cool")
    axes[4][2].set_title("KLDivLoss with $\epsilon=0.5$")
    axes[4][2].axis('off')
    ...
    axes[5][1].set_title(str(LOSS)+" with $\epsilon=0.25$")
    axes[5][1].axis('off')
    axes[5][2].imshow(softmaxed5, cmap="cool")
    axes[5][2].set_title(str(LOSS)+" with $\epsilon=0.25$")
    axes[5][2].axis('off')

    axes[6][0].imshow(np.moveaxis(input_target.cpu().squeeze().numpy(), 0, -1))
    axes[6][0].set_title("")
    axes[6][0].axis('off')
    axes[6][1].imshow(activations5, cmap="cool")
    axes[6][1].set_title(str(LOSS)+" with $\epsilon=0.1$")
    axes[6][1].axis('off')
    axes[6][2].imshow(softmaxed5, cmap="cool")
    axes[6][2].set_title(str(LOSS)+" with $\epsilon=0.1$")
    axes[6][2].axis('off')
